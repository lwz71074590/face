...
if __name__  == '__main__':
    ...
    ctx = mx.gpu(0)
    detector = MtcnnDetector(model_folder=mtcnn_path, ctx=ctx, num_worker=1, accurate_landmark = True, threshold=[0.8,0.8,0.9])
    video_path = '../../../demo/video/1.mp4'
    ...
    while True:
        ...
        if ret == None:
            ...
            time1 = time.time()
            total_boxes, points = detector.detect_face(image)
            time2 = time.time()
            print(time2-time1)
            print('face num:', len(total_boxes))
